...
export async function GET(request: NextRequest) {
  try {
    ...
    const limit = await rateLimit(`analytics:ai:${organizationId}`, { windowMs: 60000, maxRequests: 60 });
    ...
    const [metrics, providers, overview] = await Promise.all([
      AIAnalytics.getMetrics(organizationId, days),
      AIAnalytics.getProviderUsage(organizationId, days),
      AIAnalytics.getOverview(organizationId, days),
    ]);

    return NextResponse.json({ data: { metrics, providers, overview } });
  } catch (err) {
